...
def interpret_rfid_read(callbacks):
    ...
    if global_activate_time and datetime.now() - global_activate_time > timedelta(
        0, 0, 10000
    ):
        blinkLed(False)
        buzz(False)
    (status, TagType) = global_mifare_reader.MFRC522_Request(
        global_mifare_reader.PICC_REQIDL
    )
    if status == global_mifare_reader.MI_OK:
        ...
        if status == global_mifare_reader.MI_OK and (
            str_uid not in readRfidCards.keys()
            or datetime.now() - readRfidCards[str_uid] > timedelta(0, 3)
        ):
...
def send_event_to_broker(uid, activate_time, registered_uid):
    ...
    mqtt_connection.call_worker(
        str(num) + "." + activate_time.strftime("%Y-%m-%d %H:%M:%S")
    )
